...
def create_simple_schedule(dates, producers, injectors, sched_fnm, wctrls, wts, mode='hist', dateshift=timedelta(1)):
   ...
   for wnm in producers.keys()+injectors.keys(): has_opened[wnm]=False 
      ...
      if wts:
         ...
         if date >= wt['date']:
            if i > 0 and wt['date'] > dates[i-1]+dateshift:
               print >>f, 'DATES\n %s /\n /\n' % wt['date'].strftime("%d '%b' %Y").upper()
            print >>f, 'WTRACER\n %s %s  %.2f /\n /\nTSTEP\n %.2f /\n/\nWTRACER\n %s %s  0. /\n/\n' % \
                 (wt['wellnm'], wt['nm'], wt['conc'], wt['tstep'], wt['wellnm'], wt['nm'])
            wts.remove(wt)
      #
      # new date
      print >>f, 'DATES\n %s /\n/\n' % ecl_date.strftime("%d '%b' %Y").upper()
      #
      ### PRODUCERS        
      #
      #   WCONHIST/WCONPROD
