...
import sys, tempfile, os, pdb, pickle, re, time
import pylab as pl
import numpy
...
def read_rft_file_old(varnm, ef, rftfile=None, nwells=1, segments=None):
    ...
    for i in range(nwells):
        indx = list(range(i,n_tsteps*nwells, nwells))
        l.append(m[indx,:])
    return l
    ...
    def __init__(self, data, varnames, units, nm):
        ...
        for varnm in varnames:
            ...
            if varnm.startswith('S'):
                ...
                segm = int(segm)
                if wname not in self._segms: self._segms[wname] = []
...
#
    def unit(self, varnm) :
        if   varnm in list(self._map.keys()): return self._map[varnm][1]
...
#
    def get(self, varnm, cumul=False, scaler=1., Bo=1, Bg=1, Bw=1, Rs=1) :
        ...
        if 'SLFR' in varnm:           # total liquid production for well segments (not available in Eclipse)
            ...
            if 'SGFR'+postfix in self._map:
                free_gas = self._data[:,self._map['SGFR'+postfix][0]] -y*Rs
                y += free_gas*Bg
            if 'SWFR'+postfix in self._map:
...
class SummaryVectors() :
    ...
    def __init__(self, nm, separator=':'):
        ...
        for varnm in list(self._sum.keys()):
            if varnm.startswith('W'):
                wname = varnm.split(separator)[1]
                if not wname in self.wells: self.wells.append(wname)
            if varnm.startswith('S'):
                ...
                if not segm in self._segms[wname]: self._segms[wname].append(segm)
        # for convinience
        if not 'TIME' in list(self._sum.keys()):
            self.time = self._sum.numpy_vector('YEARS') * 365.25
        else:
            self.time = self._sum.numpy_vector('TIME')
        self.wells.sort()
...
#
    def unit(self, varnm) :
        if varnm in list(self._sum.keys()): return self._sum.unit(varnm)
        else                        : return '-'
#
    def get(self, varnm, cumul=False, scaler=1., Bo=1, Bg=1, Bw=1, Rs=1) :
        '''
        ...
        if 'SLFR' in varnm:           # total liquid production for well segments (not available in Eclipse)
            ...
            if 'SGFR'+postfix in self._sum:
                free_gas = self._sum.numpy_vector('SGFR'+postfix) -y*Rs
                y += free_gas*Bg
            if 'SWFR'+postfix in self._sum:
    ...
    def contour_plot_blocks(self, varnm, blocks,
        ...
        if accum:
            for i in range(len(blocks)):
                z[i,:] = UT.cumulative(self.time, z[i,:])
        ax =  PU.contourf(list(range(len(blocks))), self.time, z.T, zmin=zmin, zmax=zmax)
...
def _read_3D_data_old(casenm, nx, ny, nz, file_ext):
    ...
    for line in f:
        if searchstr in line:
            ...
            data = pl.zeros((nx,ny,nz))
            if varname not in alldata: alldata[varname] = []
...
#
    def varnames(self):
        return [nm for nm in list(self.__dict__.keys()) if not nm.startswith('_')]
...
def create_vfp_table2(q, q_func, wc, wc_func, dp_max, ref_depth, shift=0.,table_number=1, fname=None):
 ...
 -- wc-function: %s
  ...
  0. /                        gfr
  0. /                        alq
 ''' % (q_func.__name__, wc_func.__name__, table_number, ref_depth, q_, wc_)
...
def create_vfp_table3(q, q_func, wc, wc_func, gfr, gfr_func, dp_max, ref_depth, shift=0.,table_number=1, fname=None, thp=0.):
 ...
 -- wc-function: %s
  ...
  %s / gfr
  0. / alq
 ''' % (q_func.__name__, wc_func.__name__, table_number, ref_depth, q_, thp, wc_, gfr_)
...
def create_vfp_table4(s):
 ...
 -- dp-function : %s
  ...
  %s / gor
  0. / alq
 ''' % (s.vps, s.dp_f.__name__, s.table_number, s.ref_depth, q_, thp_, wct_, gor_)
...
def create_vfp_table5(q, q_func, wc, wc_func, dpo_max, dp_cut, ref_depth, table_number=1, fname=None, thp=0., dp_xtra=0.):
 ...
 -- wc-function: %s
  ...
  0. /                        gfr
  0. /                        alq
 ''' % (q_func.__name__, wc_func.__name__, table_number, ref_depth, q_, thp, wc_)
    dp = pl.zeros((len(wc), len(q)))
    # for clarity...
...
def create_vfp_table6(q, q_func, gvf, gvf_func, dpo_max, dp_cut, ref_depth, table_number=1, fname=None, thp=0., dp_xtra=0.):
 ...
 -- wc-function: %s
  ...
  %s /                        gor
  0. /                        alq
 ''' % (q_func.__name__, gvf_func.__name__, table_number, ref_depth, q_, thp, glr_)
...
def _writeProd(traj, wname):
    ...
    print("----------------------------------------", file=f)
    print("WELL %s /" % (wname), file=f)
    print("----------------------------------------", file=f)
    print("WELSPECS  COMPDAT WELSEGS", file=f)
    print("OIL", file=f)
    print("RWE 0.108 /", file=f)
    print("WELLXYZ", file=f)
    for p in traj:
        print("  %.1f   %.1f   %.1f" % (p[0],p[1],p[2]), file=f)
    print("/", file=f)
    f.close()
...
def create_well_schedule(casenm, traj_fname, wname, is_ijk=True, diam=0.1, roughness=1e-5, transf_by_mapaxes="Yes"):
    ...
    f = open(tmp_fn, 'w')
    print("PROCESS INIT DATA", file=f)
    print("DEFINE WELL CONNECTION DATA", file=f)
    print("Global", file=f)
    print(well_definition, file=f)
    print(casenm, file=f)
    print(output_fn, file=f)
    print(transf_by_mapaxes, file=f)
    f.close()
...
def shift_schedule_section(datafile, box, fname=None, sched_only=True, remove_after_delim=True):
    ...
    all_kws = [x.split('-')[0] for x in list(dd._chunks.keys())]
...
def actions_for_onoff_valves(wellnm, segm_var, lim, sched_file, fname=None):
    ...
    n_segm = len(compdats)
    segments = list(range(n_segm+2, 2*n_segm+2))
...
#
    def run(self, run_mode='local', ecl_version='2016.2'):
        ...
        while True:
            ...
            if self.trn: print('tstep=', tstep, file=self.trn)
            if tstep:
...
def create_simple_schedule(dates, producers, injectors, sched_fnm, wctrls, wts, mode='hist', dateshift=datetime.timedelta(1), header=[]):
    ...
    # first: remove those tracers that are not in given time-span
    for wt in wts[:]:  # careful not to edit looping list..
        if not dates[0] <= wt['date'] <= dates[-1]: wts.remove(wt)
    #
    for i,date in enumerate(dates):
        ecl_date = date+dateshift    # note: adding 1 day to get from last day in month to first. more convinent
        # handling tracers..
        # note1: we inject double the rate in half a day (to avoid trouble if tracer has been injected on last day
        #       of month or if we create daily schedule
        # note2: it's tricky to handle first and last day of the month. hope this works...
        # note3: will fail if more than one well had an injection on one day
        if wts:
            wt = wts[0]
            if date >= wt['date']:
                if i > 0 and wt['date'] > dates[i-1]+dateshift:
                    print('DATES\n %s /\n /\n' % wt['date'].strftime("%d '%b' %Y").upper(), file=f)
                print('WTRACER\n %s %s  %.2f /\n /\nTSTEP\n %.2f /\n/\nWTRACER\n %s %s  0. /\n/\n' % \
                     (wt['wellnm'], wt['nm'], wt['conc'], wt['tstep'], wt['wellnm'], wt['nm']), file=f)
                wts.remove(wt)
        #
        # new date
        print('DATES\n %s /\n/\n' % ecl_date.strftime("%d '%b' %Y").upper(), file=f)
        #
        ### PRODUCERS
        #
        #   WCONHIST/WCONPROD
        ...
        write_keyw = True
        for wnm,w in list(producers.items()):
            ...
            if write_keyw:
                print(keyw, file=f)
                write_keyw=False
            print(templ % (wnm, wctrls[wnm], wopr, wwpr, wgpr_str, wbhp), file=f)
        if not write_keyw: print('/\n', file=f)
        #
        ### INJECTORS
        #
        #   WCONINJH / WCONINJE
        ...
        write_keyw = True
        for wnm, w in list(injectors.items()):
            if not date in w.index: continue
            wwir, wbhp, wconc = w.loc[date]
            if wwir < 0. or wbhp < 0.:                                  # skip negative values
                continue
            if not has_opened[wnm] and abs(wwir)<RATE_EPS:
                continue
            has_opened[wnm] = True
            if write_keyw:
                print(keyw, file=f)
                write_keyw=False
            print(templ % (wnm, wwir, wbhp), end=' ', file=f)
        if not write_keyw: print('/\n', file=f)
        #
        #   WPOLYMER
        for wnm,w in list(injectors.items()):
            if not has_opened[wnm]: continue
            print('WPOLYMER\n   %s  %.1f 0.0 /\n/\n' % (wnm, wconc), file=f)
    #
    f.close()
