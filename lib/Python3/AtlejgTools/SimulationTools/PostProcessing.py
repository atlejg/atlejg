...
def _xplot(varnm1, varnm2, cases, refcase=None, plot_kwargs=None, nolegend=False):
    ...
    grid(True)
    if varnm2.startswith('WMCTL'):
        yticks(list(range(-7,14)), ('bhp (grp)', 'thp (grp)', 'resv (grp)', 'lrat (grp)',   # grp = group control
...
def _get_varnms(wname_pattern, varnm, case):
    s = get_summary(case)
    ptrn = '%s%s%s' % (varnm, ECL.SEPARATOR, wname_pattern)
    varnms = [v for v in s.varnames() if fnmatch.fnmatch(v, ptrn)]
    varnms.sort()
    return varnms
...
def _wxplot(varnm1, varnm2, wname_pattern, case, plot_kwargs=None, nolegend=False):
    ...
    grid(True)
    if varnm2.startswith('WMCTL'):
        yticks(list(range(-7,8)), ('bhp (gr)', 'thp (gr)', 'resv (gr)', 'lrat (gr)',   # gr = group control
...
def _segment_data_at_given_time(varnm, cases, t0, plot_kwargs=None, adjust_to_zero=False, use_icd_segm=True, accum=False):
    ...
    for case in cases:
        ...
        if accum:
            tixs = list(range(tix))   # time indices
...
def _connection_data_at_given_time(varnm, cases, t0, dx, ijks, plot_kwargs=None):
    ...
    for case in cases:
        ...
        if UI.accum: tixs = list(range(t0_indx))   # time indices
        else       : tixs = (t0_indx,)
        y_ = 0
...
def _fluidheight_contours(case, fluid, tsteps, hmin, hmax, rel_h):
    ...
    if not tsteps: tsteps = list(range(len(r3.time)))
    for tstep in tsteps:
...
def _statistics(varnm, t0, plot_all, ref_value, descriptions, case_groups):
    ...
    for case_group in case_groups:
        ...
        i += 1
    xticks(list(range(i)), descriptions)
    if relative > 0:
