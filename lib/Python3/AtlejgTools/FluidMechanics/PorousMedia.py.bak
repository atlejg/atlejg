...
def dp_radialflow(q, mu, k, rw, lw, rr):
   '''
   dupuit-thiem equation.
   calculates dP of radial flow from/to a well
   * input
      q : flow rate [m3/d]
      ...
      rw: well radius [m]
      lw: well length [m]
      rr: reservoir radius [m]
   * output
      dp: [bar]
   '''
   qq = q/U.CM**3 / U.DAY / (lw/U.CM)  # in 'darcy units', cm2/s
   return qq*mu*numpy.log(rr/float(rw)) / (2*numpy.pi*k) * U.ATM / U.BAR
