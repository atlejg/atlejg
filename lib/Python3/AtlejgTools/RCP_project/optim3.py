...
#
    def get_vals(self, val, line_from, line_to):
        ...
        keys = list(self.fp.__dict__.keys())
        if val in keys:
...
if False:
    for label in list(it.data.keys()): it.plot(label, rcp_params1)
    UT.hardcopies(12, 'Figs', 'NK_all_')
    close('all')
    for label in list(it.data.keys()): it.plot(label, rcp_params2)
    UT.hardcopies(12, 'Figs', 'NK_singlephase_')
    close('all')
    for label in list(it.data.keys()): it.plot(label, rcp_params3)
