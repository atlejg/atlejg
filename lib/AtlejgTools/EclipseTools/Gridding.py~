...
def get_coord(Lx, x0, dx1, k, dx_max):
   ...
   while True:
      ...
      if dx > dx_max:
         # make sure we dont get the next to last coordinate squeezed into the last.
         # we do this by making homogenous prid on the remaining part
         dL = x
         n = int(pl.ceil(dL / dx_max))
         dx = dL / float(n)
         for i in range(1, n+1):
...
-- filename = %s''' %
   ...
   i_indices = range(nx) + list(pl.arange(nx)+1)
   i_indices.sort()
   j_indices = range(ny) + list(pl.arange(ny)+1)
   j_indices.sort()
   k_indices = range(nz) + list(pl.arange(nz)+1)
   k_indices.sort()
   for k in k_indices:
      for j in j_indices:
         for i in i_indices:
