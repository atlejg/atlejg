...
#
   def get_branches(self, bnames):
      ...
      '''
      if not 'well' in self.__dict__.keys():
...
def _fix_varnm(varnm):
   ...
   varnm = varnm.replace(u'\u03bc', 'mu')   # viscosity -> mu
   varnm = varnm.replace(u'\u03c1', 'rho')  # density -> rho
...
def read_ntlcase(ntlcase):
   ...
   for line in lines:
      ...
      for varnm in varnames:
         if not obj.__dict__.has_key(varnm):
