...
UI.COMP_VARS    = ['FLPR', 'WBHP', 'SLFR', 'SPRD', 'SWCT']  # variables to be plotted with the 'compiled' functionality
...
def _segment_data_at_given_time(varnm, cases, t0, plot_kwargs=None, adjust_to_zero=False, use_icd_segm=True):
   global y_
   figure()
   ...
   for case in cases:
      ...
      r = get_summary(case)
      if t0 > 0: maxind = UT.find_closest_index(r.time, t0)
      else     : maxind = int(t0)
      ...
      if UI.accum: indices = range(maxind)
      else       : indices = (maxind,)
      y_ = 0
      for ind in indices:
         if UI.accum:
            if ind > 0: dt = r.time[ind] - r.time[ind-1]
            else      : dt = r.time[ind]
         ...
         if varnm == 'SGVF':
            ...
            y_ += dt*g/(g+o+w) # this is gvf
         else:
            y_ += dt*r.get_segm_data(varnm, WELLNM, segments, scaler=scaler)[:, ind]
      if varnm == 'SGVF': ylbl = 'SGVF [-]'
      else              : ylbl = '%s [%s]' % (varnm, r.unit('%s%s%s%s%i'%(varnm, ECL.SEPARATOR, WELLNM, ECL.SEPARATOR, segments[0])))
      if   plot_kwargs              : kwargs = plot_kwargs(case, cases)
...
def _connection_data_at_given_time(varnm, cases, t0, dx, plot_kwargs=None):
   ...
   for case in cases:
      ...
      if UI.accum: indices = range(t0_indx)
      else       : indices = (t0_indx,)
      y_ = 0
...
def _compiled(cases, segm1, segm2, plot_kwargs=None):
   ...
   _xplot('TIME', 'FOPT', cases, plot_kwargs=UI.plot_kwargs, nolegend=True)
   if UI.legend: UI.legend(cases)
   savefig('xxx001_FOPT1.png')
   _xplot('FWPT', 'FOPT', cases, plot_kwargs=UI.plot_kwargs, nolegend=True)
   savefig('xxx002_FOPT2.png')
   n = 2
   for varnm in UI.COMP_VARS:
      if varnm.startswith('S'):
         _xplot('TIME', '%s%s%s%s%i'%(varnm, ECL.SEPARATOR, WELLNM, ECL.SEPARATOR, segm1), cases, plot_kwargs=UI.plot_kwargs, nolegend=True)
         n += 1
         savefig('xxx%03i_%s_%i.png' % (n, varnm, segm1))
         _xplot('TIME', '%s%s%s%s%i'%(varnm, ECL.SEPARATOR, WELLNM, ECL.SEPARATOR, segm2), cases, plot_kwargs=UI.plot_kwargs, nolegend=True)
         n += 1
         savefig('xxx%03i_%s_%i.png' % (n, varnm, segm2))
      elif varnm.startswith('W'):
         _xplot('TIME', '%s%s%s'%(varnm, ECL.SEPARATOR, WELLNM), cases, plot_kwargs=UI.plot_kwargs, nolegend=True)
         n += 1
         savefig('xxx%03i_%s.png' % (n, varnm))
      else:
         _xplot('TIME', varnm, cases, plot_kwargs=UI.plot_kwargs, nolegend=True)
         n += 1
         savefig('xxx%03i_%s.png' % (n, varnm))
   # put together to one figure
   prefix = cases[0][0] # typically 'A' (from A1.DATA)
   fname = '%s_collage.png' % prefix
   UT.make_collage(UT.glob('xxx*.png'), 1+len(UI.COMP_VARS),2, newfile=fname)
...
def analyze(mode, *args):
   ...
   if mode == 'run'  : 
      ...
      t0             = args[1]          # in days or -1 for last timestep
      ...
      _segment_data_at_given_time(
            varnm, cases, t0, plot_kwargs=UI.plot_kwargs, adjust_to_zero=adjust_to_zero, use_icd_segm=use_icd_segm)
      if UI.legend: UI.legend(cases)
   elif mode == 'segments2':
   ...
   show()  # on ipython 0.12, this must be done explicitely:-(
